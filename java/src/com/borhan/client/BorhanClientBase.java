...
abstract public class BorhanClientBase implements Serializable {
	...
	public void queueServiceCall(String service, String action, BorhanParams kparams) {
	...
	public void queueServiceCall(String service, String action, BorhanParams kparams, BorhanFiles kfiles, Class<?> expectedClass) {
		// in start session partner id is optional (default -1). if partner id was not set, use the one in the config
		if (!kparams.containsKey(PARTNER_ID_PARAM_NAME))
			kparams.add(PARTNER_ID_PARAM_NAME, this.borhanConfiguration.getPartnerId());
	...
	private PostMethod createPostMethod(BorhanParams kparams,
			BorhanFiles kfiles, String url) {
	...
	private String extractParamsFromCallQueue(BorhanParams kparams, BorhanFiles kfiles) throws BorhanApiException {
		...
		String url = this.borhanConfiguration.getEndpoint() + "/api_v3/index.php?service=";
		
		// append the basic params
		kparams.put("apiVersion", this.getApiVersion());
		kparams.put("clientTag", this.borhanConfiguration.getClientTag());
	...
	private PostMethod addParams(PostMethod method, BorhanParams kparams) {
	...
	public String generateSession(String adminSecretForSigning, String userId, BorhanSessionType type, int partnerId) throws Exception
	{
		return this.generateSession(adminSecretForSigning, userId, type, partnerId, 86400);
	}
	
	public String generateSession(String adminSecretForSigning, String userId, BorhanSessionType type, int partnerId, int expiry) throws Exception
	{
		return this.generateSession(adminSecretForSigning, userId, type, partnerId, expiry, "");
	}

	public String generateSession(String adminSecretForSigning, String userId, BorhanSessionType type, int partnerId, int expiry, String privileges) throws Exception
	{
		try
		{
	...
	public String generateSessionV2(String adminSecretForSigning, String userId, BorhanSessionType type, int partnerId, int expiry, String privileges) throws Exception
	{
		try {
		// build fields array
