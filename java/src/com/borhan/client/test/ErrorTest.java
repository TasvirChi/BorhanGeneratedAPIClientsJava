...
public class ErrorTest extends BaseTest {
	 ...
	 */
	public void testEmptyObject() throws BorhanApiException {
		BorhanClientMock mockClient = new BorhanClientMock(this.borhanConfig, "<xml><result><objectType>BorhanPlaylist</objectType><filters><item/></filters></result></xml>");
		mockClient.queueServiceCall("system", "ping", new BorhanParams());
		Element resultXmlElement = mockClient.doQueue();
		try {
			BorhanObjectFactory.create(resultXmlElement, null);
	...
	public void testArrayOfUknownEntry() throws BorhanApiException {
		...
		try {
			...
			BorhanMediaEntry entry1 = res.objects.get(0);
			BorhanMediaEntry entry2 = res.objects.get(1);
